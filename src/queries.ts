...
export function useCreateColumnMutation() {
    const mutationFn = useConvexMutation(api.board.createColumn).withOptimisticUpdate((localStore, args) => {
...
export function useCreateItemMutation() {
    const mutationFn = useConvexMutation(api.board.createItem).withOptimisticUpdate((localStore, args) => {
        const board = localStore.getQuery(api.board.getBoard, { id: args.boardId });
        if (!board) return;
...
export function useUpdateCardMutation() {
    const mutationFn = useConvexMutation(api.board.updateItem).withOptimisticUpdate((localStore, args) => {
        const board = localStore.getQuery(api.board.getBoard, { id: args.boardId });
        if (!board) return;
...
export function useDeleteCardMutation() {
    const mutationFn = useConvexMutation(api.board.deleteItem).withOptimisticUpdate((localStore, args) => {
        const board = localStore.getQuery(api.board.getBoard, { id: args.boardId });
        if (!board) return;
...
export function useDeleteColumnMutation() {
    const mutationFn = useConvexMutation(api.board.deleteColumn).withOptimisticUpdate((localStore, args) => {
        const board = localStore.getQuery(api.board.getBoard, { id: args.boardId });
        if (!board) return;
...
export function useUpdateColumnMutation() {
    const mutationFn = useConvexMutation(api.board.updateColumn).withOptimisticUpdate((localStore, args) => {
        const board = localStore.getQuery(api.board.getBoard, { id: args.boardId });
        if (!board) return;
