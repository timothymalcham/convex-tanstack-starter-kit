...
export function SidebarLayout({
    ...
    return (
        <SidebarLayoutContext.Provider value={contextValue}>
            <div
                className={twMerge("flex h-screen flex-col bg-surface", className)}
...
function SidebarLayoutHeader({ children, className, showMenuButton = true }: SidebarLayoutHeaderProps) {
...
function SidebarLayoutSidebar({ children, className, width = "md" }: SidebarLayoutSidebarProps) {
...
function SidebarLayoutMain({ children, className, padding = true }: SidebarLayoutMainProps) {
    return <main className={twMerge("flex-1 overflow-y-auto", padding && "p-4 lg:p-6", className)}>{children}</main>;
}
...
function SidebarLayoutFooter({ children, className }: SidebarLayoutFooterProps) {
    return (
        <footer className={twMerge("border-t border-border-outline bg-surface-card px-4 py-4 lg:px-6", className)}>
            {children}
        </footer>
    );
}
...
function SidebarLayoutBody({ children, className }: SidebarLayoutBodyProps) {
    return <div className={twMerge("flex flex-1 overflow-hidden", className)}>{children}</div>;
}
