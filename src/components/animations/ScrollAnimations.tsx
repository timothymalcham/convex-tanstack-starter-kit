...
export function RevealOnScroll({ children, className, threshold = 0.1, triggerOnce = true }: RevealOnScrollProps) {
    return (
        <motion.div
...
export function ScrollCounter({ from, to, duration = 2, className, suffix = "", prefix = "" }: ScrollCounterProps) {
        ...
        <motion.div
            ...
            onViewportEnter={() => {
                ...
                const interval = setInterval(() => {
                    ...
                    setCount((prev) => {
                        const next = prev + Math.ceil((to - from) / (duration * 60));
...
export function Typewriter({ text, delay = 0, speed = 50, className, showCursor = true }: TypewriterProps) {
...
export function MorphingShape({ shape1, shape2, className, duration = 2 }: MorphingShapeProps) {
    return (
        <motion.svg
            className={className}
            viewBox="0 0 100 100"
            ...
            viewport={{ once: false }}
        >
            <motion.path d={shape1} fill="currentColor" />
        </motion.svg>
    );
}
...
export function FloatingElement({ children, intensity = 10, speed = 4, className }: FloatingElementProps) {
    return (
        <motion.div
            animate={{
