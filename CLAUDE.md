...
- Always consider multiple approaches and choose the best solution. The "best solution" often means the most elegant, simplest, and easy-to-understand solution.
- Always write code that junior engineers and new developers should be able to understand. 
- Don't ever write code that is confusing, overly complex, or more verbose than needed.
- Work off of the TODO.md file when starting a task and mark todo items in TODO.md as complete as you finish them.
...
- `npm run dev` - Start development with Convex backend and frontend in parallel
- `npm run seed` - Seed the database with sample data from sampleData.jsonl
- `npm run build` - Build the application for production
- `npm run start` - Start the production server

Individual commands:
- `npm run dev:convex` - Start only the Convex backend
- `npm run dev:start` - Start only the frontend (vinxi dev)
...
**Frontend Stack:**
- TanStack Start for SSR framework
...
- Convex handles database, real-time subscriptions, blob storage, and API endpoints
- File-based API routing in `convex/` directory

**Key Integration Points:**
- `app/router.tsx` configures the router with Convex integration via ConvexQueryClient
- Routes use `convexQuery()` helper with `useSuspenseQuery()` for data fetching
- Convex functions are imported from `convex/_generated/api`
...
- `convex/_generated/` - Auto-generated types and API definitions
- `app/routes/` - File-based frontend routes
- `app.config.ts` - TanStack Start configuration

**Data Flow:**
- Frontend components use `convexQuery(api.functionName, args)` with TanStack Query
- Convex functions are defined with explicit validators and return types
- Real-time updates work automatically through Convex subscriptions

## Environment Setup

Requires `VITE_CONVEX_URL` environment variable for Convex connection.
